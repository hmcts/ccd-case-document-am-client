...
@TestPropertySource(properties = "case_document_am.url=http://localhost:5171")
class CaseDocumentManagementClientAutoConfigurationTest {

    @Autowired
    private ApplicationContext context;

    @Test
    void shouldLoadAutoConfigurationFromImportsFile() {
        assertThat(Thread.currentThread().getContextClassLoader().getResource(
            "META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports"))
            .as("Boot 4 auto-configuration must be registered in AutoConfiguration.imports")
            .isNotNull();
    }

    @Test
    void shouldRegisterFeignClientsAndHealthIndicatorWhenUrlIsSet() {
        assertThat(context.getBean(CaseDocumentClientApi.class)).isNotNull();
        assertThat(context.getBean(CaseDocumentHealthClientApi.class)).isNotNull();
        assertThat(context.getBean(CaseDocumentManagementHealthIndicator.class)).isNotNull();
    }
}
